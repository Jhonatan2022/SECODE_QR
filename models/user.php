...
function getClinicData($id,$isnew, $codigo) {
  ...
  if(!$isnew){
    ...
    $query = $connection->prepare('SELECT qr.Titulo, us.Nombre, us.FechaNacimiento, eps.NombreEps, us.Telefono , us.Correo, us.Genero,dta.TipoAfiliacion,dta.RH, dta.Tipo_de_sangre, dta.CondicionClinica, dta.AlergiaMedicamento
    FROM usuario AS us LEFT OUTER JOIN eps 
    ON eps.id = us.id 
    LEFT OUTER JOIN datos_clinicos AS dta
    ON us.Ndocumento= dta.NDocumento
